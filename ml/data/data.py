...
class Data:
    def __init__(self, config: Config, spark_reader: SparkReader, is_train=True):
    ...
    def master_df(self) -> DataFrame:
        ...
        if self.is_train:
            master_df.write.format("parquet").save(
                self.datalake_url + self.train_data_path + time.strftime("%Y%m%d%H%M%S")
            )
            self.logger.info("Train master data is fetched and saved.")
        else:
            master_df.write.format("parquet").save(
                self.datalake_url
                + self.predict_data_path
